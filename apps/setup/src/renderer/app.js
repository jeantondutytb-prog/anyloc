...
async function teleportSpot(spot) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (map) map.setView([spot.lat, spot.lng], 12);
      updateStatusDashboard();
      status.textContent = `Destination activée : ${spot.name}`;
      status.className = "spots-status ok";
      showToast(`GPS appliqué : ${spot.name}`, "ok");
      setTimeout(() => switchTab("statut"), 600);
    } else {
      status.textContent = "Erreur lors de l'activation";
...
async function guideInstallApp() {
  ...
  if (result.ok) {
    markIphoneInstalled();
    ensureGuideAutoSync();
    text.textContent = "App installée !";
    hint.textContent = "Passe à l'étape suivante sur ton iPhone.";
    icon.className = "guide-status-icon ok";
    icon.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/></svg>';
    statusBox.className = "guide-status-box ok";
    btn.hidden = true;
    setTimeout(() => guideNext(), 800);
  } else {
    text.textContent = "Échec de l'installation";
...
async function init() {
  ...
  try {
    ...
    // Spots tab may be unavailable during early init.
  }
  void applyPlatformHints();
  ...
  $("spots-grid")?.addEventListener("click", (e) => {
    ...
    if (spot) void teleportSpot(spot);
  });
